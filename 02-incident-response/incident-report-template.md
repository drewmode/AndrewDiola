...
This case covers the part of SOC work that happens after detection: deciding what matters, tracking it through to resolution, and writing it up clearly enough that another analyst could pick up the investigation where you left off. The workflow includes triage, investigation, documentation, and closure—each step critical to maintaining institutional knowledge and supporting incident response at scale.
...
|---|---|---|
| **P1 – Critical** | Active compromise, confirmed data exposure | [TKT-007](./sample-tickets.md#tkt-007--large-data-transfer-to-external-ip-via-http-post) — Large data transfer to external IP via unencrypted HTTP POST |
| **P2 – High** | Strong indicator of compromise, no confirmed impact yet | [TKT-008](./sample-tickets.md#tkt-008--scheduled-task-created-by-non-admin-user-for-persistence) — Scheduled task created by non-admin user with suspicious naming |
| **P3 – Medium** | Suspicious but plausible benign explanation | [TKT-001](./sample-tickets.md#tkt-001--abnormal-port-scan-activity-against-web-server) — Abnormal port scan activity against web server during business hours |
| **P4 – Low** | Informational / noise, logged for trend tracking | [TKT-010](./sample-tickets.md#tkt-010--drive-by-download-via-malicious-ad-network-redirect) — Drive-by download via malicious ad network redirect chain |
